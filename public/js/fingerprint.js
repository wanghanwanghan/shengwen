...
//var issOnlineUrl_zhijingmai = null;
...
function loadXml(url)
{
	...
	//给issOnlineUrl_zhijingmai赋值
	issOnlineUrl_zhijingmai = window.location.protocol + "//"+serverIp+":"+getServerPort()+"/ISSOnline";
}
...
function showFVComparison()
{
	var hrefStr = "<button>指静脉比对</button>";
	$("#comparison").html(hrefStr);
}
...
function getWebServerInfo(paramArray, isFPLogin, type)
{
	$.ajax( {
		type : "GET",
		url : issOnlineUrl_zhijingmai+"/info",
		dataType : "json",
		async: true,
		//timeout:1000,
...
function getWebServerInfoForSimple(paramArray, isFPLogin, type)
{
	...
	if (xDomainRequest) 
	{  
		xDomainRequest.open('GET', issOnlineUrl_zhijingmai+"/info?random="+getRandomNum());  
...
function showFVRegister(paramArray)
{
	...
	hrefStr = "<button id='fvRegister' onclick='submitFVRegister("+param+", true"+")' title='"+paramArray[3]+"' class='linkStyle'>"+paramArray[3]+"</button>";
...
{
	$("#fvCountMessage").text(text +" "+ fvIdCount);
}
...
function submitFVRegister(title, fpCount, saveText, downloadText, isDriverInstall)
{
	...
	if (typeof(Worker) !== "undefined" && isDriverInstall)   
	{  
		var box=document.getElementById("zhijingmai_box");
	    var bg=document.getElementById("bg");
	   	box.style.display="block";//显示内容层，显示覆盖层
	 	box.style.left=parseInt((document.documentElement.scrollWidth-box.offsetWidth)/2)+document.documentElement.scrollLeft+"px";
	 	box.style.top=Math.abs(parseInt((document.documentElement.clientHeight-box.offsetHeight)/2))+document.documentElement.scrollTop+"px"; //为内容层设置位置
	...
	else if(browserFlag == "clearForVerify" || browserFlag == "clearForRegister")
	{
		...
		img.onload=function() {
	        ...
	        else if(browserFlag == "clearForRegister")
	        {
	        	...
	        	{
	        		imgGif = new Image();
		    		imgGif.src = "/public/img/base_fv0.png";
		        	clearInterval(baseShowImageInterval);
...
function checkFPReader(context, paramArray, browserFlag)
{
	...
	else if(browserFlag == "simple")
	{
		...
		if (xDomainRequest) 
		{  
			xDomainRequest.open('GET', issOnlineUrl_zhijingmai+"/beginCapture?type=1&random="+getRandomNum());  
			xDomainRequest.onload = function()
			{
				//获取接口返回值
				var resultData = xDomainRequest.responseText;
...
function isFVContains(fvIdArray, num)
{
	var fvId,tempFvId;
	var isCollected = false;
	for(var j=0; j<fvIdArray.length; j++)
	{
...
//		showImage(context, "${base}/base/images/base_fpVerify_clearImage.png", "clearForVerify");
	...
	{
		showImage(context, "/public/img/base_fpVerify_clearImage.png", "clearForRegister");
	}
...
function getFPTemplate(paramArray, flag)
{
	...
	$.ajax( {
		type : "GET",
		url : issOnlineUrl_zhijingmai+"/fingervein/getTemplate?random="+getRandomNum(),
		dataType : "json",
		async: false,
		success : function(result) 
		{
 ...
 */
var saveData = function(result, fpCount)
{
	if(collectFlag)
	{
		//取消采集
...
function close(){
	$("#bg").css("display", "none");
	$("#zhijingmai_box").css("display", "none");
...
var delFVData = function(result, context, browserFlag)
{
	var fvId;
	if(result)
	{
		...
		if(browserFlag == "simple")
		{
			document.getElementById("fv" + fvId).checked = false;
		}
		else if(browserFlag == "html5")
		{
			//将手指颜色改变--重画时也要判断
	...
	else
	{
		if(browserFlag == "simple")
		{
			document.getElementById("fv" + fpIdNum).checked = true
			collectFlag = true;
		}
		else if(browserFlag == "html5")
		{
